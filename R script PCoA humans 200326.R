...
# 1. Load your data
# ------------------------------
data_path <- "C:/Users/gaute/OneDrive/Dokumenter/Postdoc 2026-2029/Targeted SCFA/Quant data humans for PCoA.csv"
...
p <- ggplot(pcoa_coords, aes(x = PCoA1, y = PCoA2, color = Group, fill = Group)) +
  ...
  geom_text_repel(data = vecs_scaled,
                  ...
                  max.overlaps = Inf,
                  inherit.aes = FALSE,
                  nudge_x = vecs_scaled$PCoA1 * 0.1,
                  nudge_y = vecs_scaled$PCoA2 * 0.05) +
  
  # Colors + optional custom labels
  {
    if (use_custom_labels) {
      list(
        scale_color_manual(values = c("#6FB1CC",
                                      "#9A83B7",
                                      "#1F8F8B",
                                      "#E52E6B",
                                      "#F3971B",
        ...
        scale_fill_manual(values = c("#6FB1CC",
                                     "#9A83B7",
                                     "#1F8F8B",
                                     "#E52E6B",
                                     "#F3971B",
...
p <- p + annotate("text",
                  x = x_max * 1.8,       # adjust horizontally
                  y = y_max * 1.5,       # adjust vertically
                  label = paste0("\nR² = ", r2, "%\np = ", pval),
                  size = 5,               # font size
                  hjust = 1,
                  vjust = 1)
...
# 10. Save plot
# ------------------------------
ggsave("PCoA_mice_final.png", p, width = 10, height = 8, dpi = 300)