...
# 2. Load data
# -----------------------------
file_path <- "C:/Users/gaute/OneDrive/Dokumenter/Postdoc 2026-2029/Targeted SCFA/Quant data humans for PCoA.csv"
...
ggplot(plot_data, aes(x = Group, y = logValue, fill = Group)) +
  ...
  geom_jitter(color = "black", width = 0.15, size = 4, alpha = 0.8) +
  
  geom_text(
    data = plot_data %>%
      group_by(Group) %>%
      summarise(y = max(logValue, na.rm = TRUE) + 0.3, Letters = unique(Letters)),
  ...
  scale_fill_manual(values = c(
    "Meconium" = "#88B978",
    "Infant (<6 months)" = "#E52E6B",
    "Infant (6-12 months)" = "#F3971B",
    "Toddler (1-3 years)" = "#9B1B1B",
    "Child (3-6 years)" = "#9A83B7",
    "Child (6-18 years)" = "#1F8F8B",
    "Adult (>49 years)" = "#6FB1CC"
  )) +
